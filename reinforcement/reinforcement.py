...
class Actor_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        ...
        X = Dense(512, activation=activation)(X_input)
        X = Dense(256, activation=activation)(X)
        X = Dense(128, activation=activation)(X) #Added an extra layer, hopefully will help with Hardcore
        X = Dense(64, activation=activation)(X)
        X = Dense(32, activation=activation)(X) #Added an extra layer, hopefully will help with Hardcore
    ...
    def gaussian_likelihood(self, actions, pred): # for keras custom loss, from baselines
...
class Critic_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        ...
        V = Dense(512, activation=activation)(X_input)
        V = Dense(256, activation=activation)(V)
        V = Dense(128, activation=activation)(V) #Added an extra layer, hopefully will help with Hardcore
        V = Dense(64, activation=activation)(V)
        #V = Dense(32, activation=activation)(V) #Added an extra layer, hopefully will help with Hardcore
...
class PPOAgent:
    ...
    def __init__(self, env_name, model_name=""):
        ...
        self.max_average = -75 # when average score is above -75 model will be saved
        self.lr = 0.0004
        ...
        self.Actor_name = "BipedalWalkerHardcore-v3_PPO_Actor(TRIAL 11).h5"
        #self.Critic_name = f"{self.env_name}_PPO_Critic.h5"
        self.Critic_name = "BipedalWalkerHardcore-v3_PPO_Critic(TRIAL 11).h5"
        self.Normal_Actor_name = "BipedalWalker-v3_PPO_Actor.h5"
        self.Normal_Critic_name = "BipedalWalker-v3_PPO_Critic.h5"
        #self.loadNormal() # Training from Normal Bipedal Walker
        self.load()
    ...
    def loadNormal(self):
        self.Actor.Actor.load_weights(self.Normal_Actor_name)
        self.Critic.Critic.load_weights(self.Normal_Critic_name)
        ...
        if str(episode)[-2:] == "00":# much faster than episode % 100
            ...
            try:
                ...
                if not test:
                    ...
                    pylab.savefig(self.env_name+"(TEST).png")
            except OSError:
                pass
